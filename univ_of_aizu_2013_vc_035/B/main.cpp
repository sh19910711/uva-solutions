...
namespace solution {
  // constants
  const std::string YES = "Valid";
  const std::string NO  = "Invalid";
}
...
namespace solution {
  struct Solver: SolverInterface {
    ...
    static Int get_sum( const String& s ) {
      Int res = 0;
      for ( auto c : s ) {
        res += c - '0';
      }
    ...
    static Int get_double( const char& c ) {
      Int x = c - '0';
      x += x;
    ...
    static Int get_single( const char& c ) {
      return c - '0';
    }

    static bool check_numbers( const String& s ) {
      ...
      for ( auto i = 0; i < len; i += 2 ) {
        sum1 += get_double(t[i]);
        sum2 += get_single(t[i + 1]);
      }
      auto sum_s = to_string(sum1 + sum2);
      std::reverse(std::begin(sum_s), std::end(sum_s));
      return sum_s[0] == '0';
    }
  };
}
...
namespace solution {
  struct Solution: SolutionBase {
    ...
    bool input_query( Input* in ) {
      return std::getline(std::cin, in->S);
    }
    bool input( Input* in ) {
      return input_number(this->queries);
    }
