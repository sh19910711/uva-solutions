...
namespace solution {
  struct Finder {
    ...
    bool rec( const Int& r, const Int& c ) {
      if ( r >= N )
        return true;
      if ( c >= N )
        return rec(r + 1, 0);
    ...
    bool can_put( const Int& r, const Int& c, const char& x ) {
  ...
  typedef std::unique_ptr<Finder> FinderPointer;

  struct Solver: SolverInterface {
    const Output* solve( const Input* in, Output* out ) {
      auto finder = FinderPointer(new Finder(in));
      finder->find();
      out->N = in->N;
      out->grid = finder->G;
      return out;
  ...
  struct Solution: SolutionBase {
    bool input( Input* in ) {
      ...
      for ( auto i = 0; i < in->N; ++ i )
        if ( ! ( std::cin >> in->grid[i] ) )
          return false;
      return true;
    }
    void before_action( const int& t_id ) {
      std::cout << "Case " << t_id + 1 << ":" << std::endl;
    }
    void output( const Output* out ) {
      for ( auto i = 0; i < out->N; ++ i ) {
        std::cout << out->grid[i] << std::endl;
      }
    }
  };
}
