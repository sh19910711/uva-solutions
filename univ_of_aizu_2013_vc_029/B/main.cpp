...
namespace math {
  ...
  public:
    ...
    const BigInteger operator%( const BigInteger& n ) const { BigInteger tmp(*this); return tmp.divide(n); }
    const BigInteger operator%( const int& n ) const { BigInteger tmp(*this); return tmp.divide(n); }
...
namespace solution {
  ...
  public:
    const OutputStorage& solve( const InputStorage& in, OutputStorage& out, DataStorage& data ) {
      out.result = get_last_digit(in.N, data.S);
      return out;
    }
    
  protected:
    static Int get_last_digit( const math::BigInteger& N, const Int S[SIZE] ) {
      Int sid = ( N % MOD ).toInt();
      return S[sid];
    }
...
namespace solution {
  ...
  protected:
    ...
    void before_all_actions() {
      generate_s(this->storages->data);
    }
    
    void generate_s( DataStorage& data ) {
      math::BigInteger S = 0;
      math::BigInteger TEN = 10;
      data.S[0] = 0;
      for ( int i = 1; i < SIZE; ++ i ) {
        math::BigInteger D = i;
        math::BigInteger MD = math::mod_pow(D, D, TEN);
        S = ( S + MD ) % TEN;
        data.S[i] = S.toInt();
      }
    }
